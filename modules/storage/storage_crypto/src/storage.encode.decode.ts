...
export type InitialStorageEncodeFn = (config: CryptoConfig) => (id: string, text: string) => Promise<EncodedAndMetadata>
...
export const CryptoMetadataToUserSecretFn = (config: CryptoConfig) => async (meta: CryptoMetadata, userIdOrAdmin: UserIdOrAdmin) => {
    const {secrets, aesDecoder, derivedKey} = config
    const {user, admin, globalFingerprint, salt} = meta
...
export const defaultStorageEncodeFn: InitialStorageEncodeFn =

...
export const getAllValuesAndErrorsFromIds = async <T>(asyncStore: AsyncStore<string, T>, events: BaseEvent[]): Promise<IdToValuesAndErrors<T>> => {
...
export const getAllValuesFromIds = async <T>(asyncStore: AsyncStore<string, T>, events: BaseEvent[]): Promise<ErrorsOr<Map<string, T>>> => {
